...
### Linux 编译说明（避免 ort-sys 链接报错）

如果在 Linux / devcontainer 中出现如下报错：

`ort-sys could not link to the ONNX Runtime build in /usr`

请确保使用动态链接，并将库目录指向实际 `libonnxruntime.so` 所在目录（通常是 `/usr/lib`）：
...
说明：当 `ORT_LIB_LOCATION` 被设置为 `/usr` 且未开启动态链接时，`ort-sys` 会尝试静态链接并失败。
